...
const autoCompleteJS = new autoComplete({
	data: {
		src: async () => {
			...
			if (localStorage['landing_data']){
				const data = JSON.parse(localStorage.getItem('landing_data'))['data'];
				// Post loading placeholder text
				document
				.querySelector("#autoComplete")
				.setAttribute("placeholder", "Αναζήτησε τοποθεσία...");
			return data
			}
			else{
				// Fetch External Data Source
				const source = await fetch(
					"{% url 'ajax_landing_autocomplete' %}"
				);
				const data = await source.json();
				// Save to local storage
				//console.log(source.JSON.stringify);
				localStorage.setItem('landing_data', JSON.stringify({
					data:data
				}));
				// Post loading placeholder text
				document
	...
	resultItem: {
        ...
        ${data.match}</span>
        <span class="autoComplete_result_keys" style="display: flex; align-items: center; font-size: 13px; font-weight: 100; text-transform: uppercase; color: rgba(0,0,0,.2);">
      ${data.key}</span>`;
		}
	},
